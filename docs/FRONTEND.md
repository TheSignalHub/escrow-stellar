...
**Convert on Stellar**: Quote -> Sign -> Convert for selected Stellar token contracts through the configured AMM/broker route. Users can paste pay/receive SAC contract addresses, use XLM/USDC presets, flip direction, choose exact pay or exact receive, and run optional public route discovery. The current testnet adapter executes against the seeded Soroswap router pool and uses 1% slippage tolerance, so custom pairs require available route liquidity. This prepares the connected Stellar wallet only; escrow funding happens later from Deals via `fund_deal`.
