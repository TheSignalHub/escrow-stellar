...
**Section 2 — Convert on Stellar**: Quote -> Sign -> Convert for selected Stellar token contracts through the Stellar Broker route. Users can paste pay/receive SAC contract addresses, use XLM/USDC presets, flip direction, and choose exact pay or exact receive. The current testnet adapter executes against the seeded Soroswap router pool and uses 1% slippage tolerance, so arbitrary pairs require available pool liquidity. This prepares the connected Stellar wallet only; escrow funding happens later from Deals via `fund_deal`.
