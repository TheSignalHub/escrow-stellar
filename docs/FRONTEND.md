...
|---|---|---|---|
| 2026-07-23 11:46 BST | Wallet balance and ledger refresh UX | Updated the connected wallet header to always show both XLM and configured settlement-token balances, and changed Deals auto-sync to refresh silently so the left ledger list does not show loading skeletons on each polling interval. | `npm run build` passed in `frontend/`. |
