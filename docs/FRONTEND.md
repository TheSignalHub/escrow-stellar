...
**Section 2 — Convert on Stellar**: Quote -> Sign -> Convert for XLM -> test USDC through the Stellar Broker testnet route. The current testnet adapter executes against the seeded Soroswap router pool and uses 1% slippage tolerance. This prepares the connected Stellar wallet only; escrow funding happens later from Deals via `fund_deal`.
