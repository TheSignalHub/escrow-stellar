...
|---|---|---|---|
| 2026-07-30 00:38 BST | Admin dispute dashboard copy | Removed internal raw-event and shadow-binding warnings from the protected `/admin` open-dispute cards. Direct app-created disputes now show clean operator evidence and admin actions, while linked marketplace references appear only when present. | `npm run build` passed in `indexer/`. |
| 2026-07-29 23:33 BST | MoonPay sandbox onramp test surface | Added a `/moonpay_onramp_test` route that opens the MoonPay sandbox hosted widget for XLM delivery to the connected Stellar wallet using a publishable API key from `VITE_MOONPAY_API_KEY`. This is an isolated route check and does not replace the Stripe wallet top-up path. | `npm run build -- --logLevel warn` passed with existing large-chunk warning. |
| 2026-07-29 16:20 BST | Stripe hosted onramp launch guard | Prevented duplicate hosted Stripe/Link tabs by guarding repeated launch clicks and avoiding the `noopener` return-value fallback that could open a new tab and redirect the current tab to the same session. The backend session request now also constrains destination currency/network with Stripe's list parameters. | `npm run build -- --logLevel warn` passed in `frontend/` with existing large-chunk warning; `npm run build` passed in `indexer/`. |
| 2026-07-28 14:40 BST | NEAR Intents source-network guard | Refreshed EVM source-wallet chain state on browser focus/visibility changes and required the connected EVM wallet network to match the selected 1Click source route before live payment quotes/actions are enabled. Mismatches now show a switch-network CTA instead of stale route/network copy. | `npm run build -- --logLevel warn` passed with existing large-chunk warning. |
| 2026-07-28 12:14 BST | Stripe onramp header label | Removed the Stripe readiness mode pill from the user-facing Wallet Prep card so the header does not show test/production implementation labels. | `npm run build -- --logLevel warn` passed with existing large-chunk warning. |
