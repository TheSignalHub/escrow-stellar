...
|---|---|---|---|
| 2026-07-25 03:32 BST | Mainnet Stellar account readiness | Added production wallet-readiness gates: Wallet Prep flags inactive Stellar wallets, NEAR top-up quotes require an existing Stellar destination account, mainnet Create Deal validates active provider/connector accounts and USDC recipient trustlines before review, and Soroban transaction timeouts now include an explorer link with a longer confirmation window. | `npm run build -- --logLevel warn` passed with existing large-chunk warning. |
