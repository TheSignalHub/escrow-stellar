...
| 2026-07-23 14:55 BST | NEAR wallet top-up boundary | Clarified product copy and docs so NEAR Intents is presented as a cross-chain Stellar wallet top-up route, not as direct escrow funding. | `npm run build` passed in `frontend/`; escrow funding remains gated on user-confirmed `fund_deal` and Soroban `funded` events. |
...
| Near Intents | SDK dependency, `nearIntentsProvider`, public readiness endpoint, protected token/quote/status/deposit-tx/reconcile endpoints, `nearIntent` binding metadata, product-facing **Top Up from Another Chain** UI, browser client, env gates, and source-of-truth rules. Live readiness JSON, protected token discovery, and shadow binding evidence are now confirmed; dry/live quote evidence still needs final route inputs. NEAR status is wallet top-up evidence only; escrow funding still requires `fund_deal`. |
